...
阶段三当前提供命令 **Upload current image to S3**：将光标放在本地图片引用上执行命令，插件会上传对应 Vault 图片，并将当前引用替换为公开 Markdown 图片链接。上传失败时不会修改笔记，也不会删除本地文件；开启删除选项后，上传成功才会将原文件移入 Obsidian 回收站。

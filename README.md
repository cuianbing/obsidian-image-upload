...
阶段四支持粘贴图片自动上传：开启设置中的“粘贴图片时自动上传”后，粘贴板中的图片会保存为 Vault 本地文件并上传，成功后插入 S3 公开 Markdown 链接；关闭时保留 Obsidian 原生粘贴行为。上传失败时默认保留并插入本地图片，关闭“上传失败时回退到本地保存”后则会清理本次创建的本地文件。
